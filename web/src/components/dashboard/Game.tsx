...
const mockGameHistory = Array(50).fill(null).map((_, i) => {
    const opponents = ['martin-xiii', 'dj-ron-passano', 'Jdomenusrex', 'knight_rider', 'chess_lover99', 'tactical_genius', 'queen_gambit', 'pawn_star', 'rook_roller', 'bishop_boss'];
    const opponentRatings = [1800, 1350, 446, 550, 700, 625, 520, 830, 490, 675];
    ...
    return {
      ...
      date: `${randomDate.getMonth() + 1}/${randomDate.getDate()}/${randomDate.getFullYear()}`,
                ...
                <table className="min-w-full text-sm">
                  <thead className="text-amber-700 dark:text-amber-300 border-b border-amber-200/30 dark:border-amber-700/30">
                    <tr>
                      ...
                      <th className="hidden sm:table-cell text-left py-2 px-2">Date</th>
                  ...
                  <tbody>
                    {paginatedGames.map((game, i) => (
                      ...
                      >
                        ...
                        <td className={`py-2 px-2 font-medium ${getResultClass(game.result)}`}>
                          {game.result}
                        </td>
                        <td className="hidden sm:table-cell py-2 px-2">{game.date}</td>
            ...
            {totalPages > 1 && (
              <div className="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2">
                <div className="text-sm text-amber-700 dark:text-amber-300 order-2 sm:order-1">
                  Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredGames.length)} of {filteredGames.length} games
                </div>
        ...
        <Card className="w-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-amber-200/50 dark:border-amber-800/30 shadow-md">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="text-sm text-amber-700 dark:text-amber-300">Opponent</div>
            ...
            <div className="space-y-2">
              <div className="text-sm text-amber-700 dark:text-amber-300">Result</div>
              <div className="grid grid-cols-3 gap-2">
                <Button
                  variant={resultFilter === 'All' ? 'default' : 'outline'}
                  size="sm"
                  className={resultFilter === 'All' 

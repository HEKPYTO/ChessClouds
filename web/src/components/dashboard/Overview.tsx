...
const mockGameHistory = [
  { id: 1, opponent: 'martin-xiii', opponentRating: 1800, playerRating: 659, result: '1-0', date: '3/4/2025', moves: 83 },
  { id: 2, opponent: 'dj-ron-passano', opponentRating: 1350, playerRating: 659, result: '1-0', date: '2/26/2025', moves: 69 },
  { id: 3, opponent: 'Jdomenusrex', opponentRating: 446, playerRating: 292, result: '0-1', date: '2/18/2025', moves: 34 },
  { id: 4, opponent: 'Jdomenusrex', opponentRating: 446, playerRating: 292, result: '1-0', date: '2/18/2025', moves: 35 },
  { id: 5, opponent: 'knight_rider', opponentRating: 550, playerRating: 456, result: '1-0', date: '2/15/2025', moves: 42 },
  { id: 6, opponent: 'chess_lover99', opponentRating: 700, playerRating: 659, result: '½-½', date: '2/10/2025', moves: 67 },
  { id: 7, opponent: 'tactical_genius', opponentRating: 625, playerRating: 456, result: '0-1', date: '2/5/2025', moves: 51 },
  { id: 8, opponent: 'queen_gambit', opponentRating: 520, playerRating: 292, result: '1-0', date: '2/1/2025', moves: 29 },
  { id: 9, opponent: 'pawn_star', opponentRating: 830, playerRating: 659, result: '0-1', date: '1/28/2025', moves: 64 },
  { id: 10, opponent: 'rook_roller', opponentRating: 490, playerRating: 456, result: '½-½', date: '1/25/2025', moves: 78 },
];
...
export default function OverviewTab() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'playing': return 'bg-amber-600';
                  ...
                  <table className="min-w-full text-sm">
                    <thead className="text-amber-700 dark:text-amber-300 border-b border-amber-200/30 dark:border-amber-700/30">
                      <tr>
                        ...
                        <th className="hidden sm:table-cell text-center py-2 px-2">Type</th>
                        <th className="hidden md:table-cell text-left py-2 px-2">Date</th>
                        <th className="text-center py-2 px-2">Moves</th>
                      </tr>
                    </thead>
                    <tbody>
                      {mockGameHistory.slice(0, HISTORY_PER_PAGE).map((game, i) => (
                        <tr 
                          key={i}
                        ...
                        >
                          <td className="py-2 px-2 font-medium truncate max-w-[100px] sm:max-w-none">{game.opponent}</td>
                          ...
                          <td className="hidden md:table-cell py-2 px-2">{game.date}</td>
          ...
          <Card className="w-full bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-amber-200/50 dark:border-amber-800/30 shadow-md">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-semibold text-amber-900 dark:text-amber-100 flex justify-between items-center">
                <span>Friends</span>
                <span className="text-sm text-amber-600 dark:text-amber-400">{mockFriends.length}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <div className="relative">
                        ...
                        <div className={`absolute -bottom-1 -right-1 h-3 w-3 rounded-full border-2 border-white dark:border-slate-800 ${getStatusColor(friend.status)}`}></div>
                      </div>
                      <div className="min-w-0">
                        <div className="font-medium text-sm text-amber-800 dark:text-amber-200 truncate max-w-[120px]">{friend.name}</div>
                        <div className="text-xs text-amber-600 dark:text-amber-400">{friend.rating}</div>
                      </div>
                    </div>
